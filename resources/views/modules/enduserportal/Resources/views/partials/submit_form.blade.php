...
@else
    ...
    <form class="" method="POST" action="{{ $form_action ?? '' }}" id="eup-ticket-form">
        <div id="eup-submit-form-main-area">
            ...
            {{-- @if (!\EndUserPortal::authCustomer())
            <div class="form-group {{ $errors->has('ticketcategory') ? ' has-error' : '' }}">
                <input type="text" class="form-control eup-remember input-md" name="ticketcategory" value="{{ old('ticketcategory', $values['ticketcategory'] ?? '') }}" placeholder="{{ __('Ticket Category') }}*" @if (!empty($values['ticketcategory'])) data-prefilled="1" @endif required autofocus />
                <select name="ticketcategory" class="form-control eup-remember input-md" id="ticketcategory" value="{{ old('ticketcategory', $values['ticketcategory'] ?? '') }}"  @if (!empty($values['ticketcategory'])) data-prefilled="1" @endif required autofocus>
                    <option value="cat1" selected>Ticket category</option>
                    <option value="cat2"></option>
                    <option value="cat1">a</option>
                    <option value="cat1">c</option>
                </select>

                @include('partials/field_error', ['field'=>'ticketcategory'])
            </div>
        @endif --}}
        ...
        <div class="form-group {{ $errors->has('subcategory') ? ' has-error' : '' }}">
            <input type="text" class="form-control eup-remember input-md" name="ticketcategory" value="{{ old('ticketcategory', $values['ticketcategory'] ?? '') }}" placeholder="{{ __('Ticket Category') }}*" @if (!empty($values['ticketcategory'])) data-prefilled="1" @endif required autofocus />
            <select name="subcategory" class="form-control eup-remember input-md" id="subcategory" value="{{ old('subcategory', $values['subcategory'] ?? '') }}"   @if (!empty($values['subcategory'])) data-prefilled="1" @endif required autofocus>
                <option value="cat1" disabled>Sub Category</option>
                <option value="cat2">s</option>
                <option value="cat1">s</option>
                <option value="cat1">s</option>
                <option value="cat1">s</option>
            </select>
