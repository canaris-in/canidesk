		...
		<div class="eup-container-padded clearfix">
			...
			<div class="heading eup-ticket-header">{{ $conversation->getSubject() }}
				...
				@if (\EndUserPortal::getStatusName($conversation) == 'Open')
					...
					@else
					<span class="pull-right label label-success mr-3 conv-status btn-send-text"><a href="javascript:void(0)" class="span-link" data-status="{{ App\Conversation::STATUS_ACTIVE }}" data-conv-id="{{$conversation->id}}"></a>Re-open</span>
				@endif
			</div>
		</div>
